...
let to_list (ctx, lst) =
  match ctx with Start -> lst | Right (ctx, el) -> _to_list (ctx, el :: lst)
...
let elem (ctx, lst) = match lst with [] -> None | hd :: _ -> Some hd

let remove (ctx, lst) =
  match ctx with Start -> (ctx, lst) | Right (ctx, el) -> (ctx, lst)
